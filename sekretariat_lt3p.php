...
<div class="container">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($id_barang) {
            $sql = "UPDATE sekretariat_lt3p SET 
            ...
            $sql = "INSERT INTO sekretariat_lt3p (nama_barang, merk, no_seri, ukuran, bahan, pembelian, kode_barang, jumlah_barang, keadaan_barang, keterangan_barang) 
                    VALUES ('$nama_barang', '$merk', '$no_seri', '$ukuran', '$bahan', '$pembelian', '$kode_barang', '$jumlah_barang', '$keadaan_barang', '$keterangan_barang')";
        }

        if (mysqli_query($kon, $sql)) {
            header("Location: sekretariat_lt3p.php");
            exit();
        } else {
    ...
    if (isset($_GET['id_barang'])) {
        ...
        $sql = "DELETE FROM sekretariat_lt3p WHERE id_barang='$id_barang'";
        if (mysqli_query($kon, $sql)) {
            header("Location: sekretariat_lt3p.php");
            exit();
        } else {
    ...
    <div class="table-container">
        <table id="dataTable" class="table table-bordered my-3">
            ...
            <tbody>
                <?php
                $sql = "SELECT * FROM sekretariat_lt3p ORDER BY id_barang ASC";
