...
<div class="container">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($id_barang) {
            $sql = "UPDATE pengolahan_lt2p SET 
            ...
            $sql = "INSERT INTO pengolahan_lt2p (nama_barang, merk, no_seri, ukuran, bahan, pembelian, kode_barang, jumlah_barang, keadaan_barang, keterangan_barang) 
                    VALUES ('$nama_barang', '$merk', '$no_seri', '$ukuran', '$bahan', '$pembelian', '$kode_barang', '$jumlah_barang', '$keadaan_barang', '$keterangan_barang')";
        }

        if (mysqli_query($kon, $sql)) {
            header("Location: pengolahan_lt2p.php");
            exit();
        } else {
    ...
    if (isset($_GET['id_barang'])) {
        ...
        $sql = "DELETE FROM pengolahan_lt2p WHERE id_barang='$id_barang'";
        if (mysqli_query($kon, $sql)) {
            header("Location: pengolahan_lt2p.php");
            exit();
        } else {
    ...
    <div class="table-container">
        <table id="dataTable" class="table table-bordered my-3">
            ...
            <tbody>
                <?php
                $sql = "SELECT * FROM pengolahan_lt2p ORDER BY id_barang ASC";
